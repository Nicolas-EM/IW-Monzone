...
function renderMember(member, currencyString) {
    ...
    let memberHTML = `<div class="row d-flex align-items-center p-2 member">`;
    ...
    memberHTML = `${memberHTML}
                     <div id="name-col" class="col d-flex align-items-center border-end border-light-subtle ${member.idUser == userId ? 'fw-bold' : ''}">
                        ${member.idUser == userId ? "You" : member.username}`;
    if (member.role == "GROUP_CREATOR") {
        memberHTML = `${memberHTML}
                     <img src="/img/white-crown.png" alt="creatorIcon" title="Group creator" width="auto" height="25" class="d-inline-block align-text-top ps-3">`;
    }

    // Icon crown
    if (isGroupAdmin === 'false' && member.role == "GROUP_MODERATOR") {
        memberHTML = `${memberHTML}
                <img src="/img/crown.png" alt="adminIcon" title="Group moderator" width="auto" height="25" class="d-inline-block align-text-top ps-3">`;
    }
        ...
        if (member.role == "GROUP_MODERATOR") {
            memberHTML = `${memberHTML}
                            <button type="submit" title="Remove member as group moderator" class="btn w-auto" data-bs-toggle="modal" data-bs-target="#confirmModal" data-bs-type="removeModerator" data-bs-userId="${member.idUser}">
                                <img src="/img/crown.png" alt="Remove moderator" width="auto" height="25" class="d-inline-block align-text-top ps-2">
                            </button>
                            `;
        }
            ...
            memberHTML = `${memberHTML}
                            <button type="submit" title="Make member a group moderator" class="btn w-auto" data-bs-toggle="modal" data-bs-target="#confirmModal" data-bs-type="makeModerator" data-bs-userId="${member.idUser}">
                                <img src="/img/disabled-crown.png" alt="Make group moderator" width="auto" height="25" class="d-inline-block align-text-top ps-2">
                            </button>
                            `;
        }
    }
